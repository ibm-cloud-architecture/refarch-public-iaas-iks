...
resource "null_resource" "configure-monitoring" {
  provisioner "local-exec" {
    command                   = <<EOT
../configuration/configure-monitoring.sh "${data.ibm_container_cluster_config.cluster_config.config_file_path}" "${ibm_container_cluster.kubecluster.name}" "${ibm_resource_key.monitor_secret.credentials["Sysdig Access Key"]}" "ingest.us-south.monitoring.cloud.ibm.com"
EOT
  }
